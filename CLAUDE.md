...
- Weather providers sit behind the `WeatherProvider` interface in `src/services/providers/` (Tomorrow.io live; Pirate Weather adapter built, for a self-hosted instance on a separate weather VM — `server/deploy/weather/README.md`), selected by `WEATHER_PROVIDER`. Adapters must emit `ForecastMinute` exactly (`src/services/weather.ts`): `GET /weather` sends it to the app unchanged, so its fields, units and precipitation codes are a contract with installed app builds.
